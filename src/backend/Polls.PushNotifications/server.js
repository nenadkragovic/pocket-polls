...
const addMessageToBroadcastQueue = async (message) => {
    getTotalNumberOfSubscriptions(function(err, total) {
        ...
        publicChannel.sendToQueue('buffer-exchange',
            Buffer.from(JSON.stringify({total: total, offset: 0, message: message})));
    });
}
...
app.get('/get-subscriptions', async (req, res) => {
    ...
    if (limit == null || limit === undefined)
        limit = 100;
    getSubscriptions(offset, limit, function(result) { res.json({ record: result }) })
})
